...
PreparedStatement ps=con.prepareStatement("select CONCAT(UCASE(MID(name,1,1)),MID(name,2)),year(year),photoname,number,CONCAT(UCASE(MID(language,1,1)),MID(language,2)) from movies where name like ?");
...
<title><%= request.getParameter("name")%> Movie</title>
</head>
<body>
<nav>
...
<h1 style="margin-left:20px;"><%= rs.getString(1)%> Movie</h1>
...
<h1 style="color:#adadad; text-align:center;margin-top:130px;">Search Not Found</h1>
